package spawner 
{
	 ...
	 * @author Li Pinxian
	 */
	public class SpawnerChallenge extends Spawner
	{
		public function SpawnerChallenge() 
		{
			super();
			...
			patternGroup.push(new AssetManager.pattern5_1CSV);
			patternGroup.push(new AssetManager.pattern5_2CSV);
			
			for (var i:int = 0; i < 20; i++)
			{
				var tempEnemy:enemyChallenge = new enemyChallenge( -100, -100);
				tempEnemy.exists = false;
				enemyGroup.add(tempEnemy);
				add(tempEnemy.bloodEmitter);
				/*
				var tempEnemy2:enemy52 = new enemy52( -100, -100);
				tempEnemy2.exists = false;
				enemyGroup.add(tempEnemy2);
				add(tempEnemy2.bloodEmitter);
				
				var tempEnemy3:enemy53 = new enemy53( -100, -100);
				tempEnemy3.exists = false;
				enemyGroup.add(tempEnemy3);
				add(tempEnemy3.bloodEmitter);
				*/
				var tempBlock:blockChallenge = new blockChallenge( -100, -100);
				...
				/*
				var templight:satellite = new satellite( -100, -100);
				templight.exists = false;
				blockGroup.add(templight);
				//shadowGroup.add(templight.shadow);
				
				var tempBlock2:asteroid = new asteroid( -100, -100);
